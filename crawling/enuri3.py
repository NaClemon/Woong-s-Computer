...
def main():
    ...
    driver.get('http://www.enuri.com/list.jsp?cate=070511&tabType=1&page=1')
    vga_id = 100571
    vga_rank =1
    result = []
    print('server')
    ...
    pattern3 = re.compile("[A-Z]{3}[0-9]{3,4}")
    ...
    for k in range(20,24):
        ...
        for n in temp:
            ...
            if(bool(model_temp)==True):
                ...
                model = '수정 필요'
            time.sleep(.2)
            result.append([vga_id, title, price, company, model, memory,vga_url,vga_url2])
            print('vga_id: ',vga_id,'이름: ',title,' 가격: ',price,' 메모리: ',memory, 'model: ',model,'company: ',company,'img: ','vga_img: ',vga_url,vga_url2)
            vga_id+=1
        driver.implicitly_wait(3)

    articles = pd.DataFrame(result, columns=['vga_id', '제품명', '가격', '제조사', '모델', '메모리','vga_url','vga_url2'])
    articles.to_csv('vga_please3.csv', encoding='ms949')
    driver.quit()
